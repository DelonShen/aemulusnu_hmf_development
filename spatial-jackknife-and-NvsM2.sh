...
for i in {0..4}; do
  ...
  echo "#SBATCH --job-name=spatial-jackknife-and-NvsM-n50-${i}" >> "${job_script}"
  echo "#SBATCH --output=logs/$(date +%Y-%m-%d)-jack-and-NvsM-n50-${i}.out" >> "${job_script}"
  echo "#SBATCH --error=logs/$(date +%Y-%m-%d)-jack-and-NvsM-n50-${i}.err" >> "${job_script}"
  ...
  for j in {0..9}; do
    ...
    echo "python spatial_jackknife_and_NvsM.py Box_n50_${file_number}_1400" >> "${job_script}"
    echo "python spatial_jackknife_and_NvsM.py Box_n50_${file_number}_1400"
  done
