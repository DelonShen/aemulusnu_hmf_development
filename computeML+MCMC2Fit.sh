...
for i in {0..4}; do
  ...
  echo "#SBATCH --job-name=compute-ml-mcmc-fit-n50-${i}" >> "${job_script}"
  echo "#SBATCH --output=logs/$(date +%Y-%m-%d)-compute-ml-mcmc-fit-n50-${i}.out" >> "${job_script}"
  echo "#SBATCH --error=logs/$(date +%Y-%m-%d)-compute-ml-mcmc-fit-n50-${i}.err" >> "${job_script}"
  ...
  for j in {0..9}; do
    ...
    echo "python computeML-MCMC-fit.py Box_n50_${file_number}_1400" >> "${job_script}"
    echo "python computeML-MCMC-fit.py Box_n50_${file_number}_1400"
  done
