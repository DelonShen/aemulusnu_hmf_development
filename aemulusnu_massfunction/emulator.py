...
#         super(MultitaskGPModel, self).__init__(train_x, train_y, likelihood)
#         self.mean_module = gpytorch.means.MultitaskMean(
#             gpytorch.means.LinearMean(input_size=train_x.shape[1]), 
...
#         self.ComputedParams = {}
#         with open(emulator_loc, 'rb') as f:
#             self.model_path, self.in_scaler, self.out_scaler, self.likelihood, dummyX, dummyY = pickle.load(f)
#             print('pytorch model is at', self.model_path)
#             self.model = MultitaskGPModel(dummyX, dummyY, self.likelihood)
