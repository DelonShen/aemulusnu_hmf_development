...
for i in {0..9}; do
  ...
  echo "#SBATCH --job-name=compute-ml-fit-${i}" >> "${job_script}"
  echo "#SBATCH --output=logs/$(date +%Y-%m-%d)-compute-ml-fit-${i}.out" >> "${job_script}"
  echo "#SBATCH --error=logs/$(date +%Y-%m-%d)-compute-ml-fit-${i}.err" >> "${job_script}"
  ...
  for j in {0..9}; do
    ...
    echo "python computeMLFit.py Box${file_number}_1400" >> "${job_script}"
    echo "python computeMLFit.py Box${file_number}_1400"
  done
