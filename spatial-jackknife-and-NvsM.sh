...
for i in {0..9}; do
  ...
  echo "#SBATCH --job-name=spatial-jackknife-and-NvsM-${i}" >> "${job_script}"
  echo "#SBATCH --output=logs/$(date +%Y-%m-%d)-jack-and-NvsM-${i}.out" >> "${job_script}"
  echo "#SBATCH --error=logs/$(date +%Y-%m-%d)-jack-and-NvsM-${i}.err" >> "${job_script}"
  ...
  for j in {0..9}; do
    ...
    echo "python spatial_jackknife_and_NvsM.py Box${file_number}_1400" >> "${job_script}"
    echo "python spatial_jackknife_and_NvsM.py Box${file_number}_1400"
  done
